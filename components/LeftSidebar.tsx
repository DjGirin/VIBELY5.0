...
import { SparklesIcon, ListIcon, MessageSquareIcon, MusicIcon, DoorOpenIcon, SettingsIcon, UserIcon } from './icons';

type Page = 'stage' | 'friends' | 'music' | 'settings' | 'messages' | 'profile' | 'community' | 'threadDetail' | 'followList' | 'projectDetail' | 'dashboard' | 'projects' | 'teams' | 'studio' | 'ai_tools' | 'backstage' | 'room';
...
const LeftSidebar: React.FC<LeftSidebarProps> = ({ onNavigate, currentPage }) => {
  // 새로운 네비게이션 순서: Stage → Community → Messages → Music → Room
  const mainNavItems: NavItemType[] = [
    ...
    { id: 'backstage', label: 'Room', icon: <DoorOpenIcon className="w-6 h-6" /> },
  ];
