...
const BackstagePage: React.FC<BackstagePageProps> = ({ onNavigate }) => {
  ...
  return (
    <main className="flex-1 max-w-4xl mx-auto p-4 md:p-6 space-y-6">
      {/* 헤더 */}
      <div>
        <h1 className="text-2xl font-bold text-light-text-primary">Room</h1>
      ...
      {/* 외부 음원 서비스 연동 */}
      <section>
        <h2 className="font-bold text-lg mb-3">외부 서비스 연동</h2>
        <div className="bg-light-surface rounded-xl border border-light-border p-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              { name: 'Suno', connected: true, icon: '🎵' },
              { name: 'Udio', connected: false, icon: '🎹' },
              { name: 'AIVA', connected: true, icon: '🤖' },
              { name: 'Mubert', connected: false, icon: '🎧' },
            ].map(service => (
              <button
                key={service.name}
              ...
              >
                ...
                <span className={`text-xs ${service.connected ? 'text-green-600' : 'text-light-text-secondary'}`}>
                  {service.connected ? '연결됨' : '연결하기'}
                </span>
              </button>
            ))}
          </div>
          <p className="text-xs text-light-text-secondary mt-3 text-center">
            외부 AI 음악 서비스에서 생성한 파일을 프로젝트에 연동하세요
          </p>
        </div>
      </section>
